...
class DQN:
    ...
    def _build_network(self, l_rate=0.01):
        with tf.variable_scope(self.net_name):
            ...
            # Conv
            W1 = tf.Variable(tf.random_normal([8, 8, 1, 256], stddev=0.01))
            ...
            W2 = tf.Variable(tf.random_normal([4, 4, 256, 512], stddev=0.01))
            ...
            L3 = tf.reshape(L3, [-1, 1024 * 2 * 2])

            W4 = tf.get_variable("W4", shape=[1024 * 2 * 2, 8192],
                                 initializer=tf.contrib.layers.xavier_initializer())
            b4 = tf.Variable(tf.random_normal([8192]))
            L4 = tf.nn.relu(tf.matmul(L3, W4) + b4)
            L4 = tf.nn.dropout(L4, keep_prob=keep_prob)

            # L5 Final FC 625 inputs -> 10 outputs
            W5 = tf.get_variable("W5", shape=[8192, self.output_size],
                                 initializer=tf.contrib.layers.xavier_initializer())
